...
bad_file = ["13349.txt","16836.txt","16837.txt","18615.txt","4062.txt","7386.txt","31014.txt","4144.txt","28076.txt","30500.txt","4045.txt"]
def work(x):
    return x+21
...
def annotate_toc_for_test(dir_path, out_dir_path, out_toc_pos_file_name, out_toc_len_file_name):
    ...
    file_num = 0
    toc_annotated_df = pd.DataFrame(
        columns=['file_id', 'toc_section', 'toc_section_pos', 'toc_section_len'])
    for key in file_toc_pos.keys():
        file_id = key
        if file_num % 300 == 0:
            print('processing file num ', file_num)
        ...
        for i in range(len(pages)):
            toc = tocs[i]
            toc_annotated_df.loc[len(toc_annotated_df.index)] = [file_id, tocs[i], pages[i], lens[i]]
